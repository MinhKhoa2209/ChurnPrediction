...
class PredictionInput(BaseModel):
    gender: Literal["Male", "Female"] = Field(..., description="Customer gender (Male or Female)")
    ...
    tenure: int = Field(
        ..., ge=0, le=72, description="Number of months customer has stayed with company (0-72)"
    )
    ...
    MonthlyCharges: float = Field(
        ..., ge=0.0, description="Customer's monthly charges (non-negative float)"
    )

    TotalCharges: float = Field(
        ..., ge=0.0, description="Customer's total charges (non-negative float)"
    )
...
class SinglePredictionRequest(BaseModel):
    ...
    input: PredictionInput = Field(..., description="Customer feature data for prediction")
...
class PredictionResponse(BaseModel):
    ...
    probability: float = Field(..., ge=0.0, le=1.0, description="Churn probability (0.0 to 1.0)")
    threshold: float = Field(..., ge=0.0, le=1.0, description="Classification threshold used")
    prediction: Literal["Churn", "No Churn"] = Field(..., description="Binary prediction result")
    ...
    model_config = {
        "json_schema_extra": {
            "example": {
                ...
                "probability": 0.73,
                "prediction": "Churn",
                "input_features": {"gender": "Female", "tenure": 12, "MonthlyCharges": 70.35},
                "created_at": "2024-01-15T14:30:00Z",
            }
        }
    }
...
class PaginationMetadata(BaseModel):
    ...
    model_config = {
        "json_schema_extra": {
            "example": {"total": 1000, "page": 1, "page_size": 50, "total_pages": 20}
        }
    }
                ...
                "predictions": [
                    {
                        ...
                        "probability": 0.73,
                        "prediction": "Churn",
                        "input_features": {"gender": "Female", "tenure": 12},
                        "created_at": "2024-01-15T14:30:05Z",
                    }
                ],
                "pagination": {"total": 1000, "page": 1, "page_size": 50, "total_pages": 20},
            }
        }
    }
