...
class TrainingJobResponse(BaseModel):
    ...
    model_version_id: Optional[UUID] = Field(
        None, description="Model version ID (set when completed)"
    )
...
class TrainingJobListResponse(BaseModel):
    ...
    model_config = {"json_schema_extra": {"example": {"jobs": [], "total": 0}}}
...
class TrainingJobCreateResponse(BaseModel):
    ...
    model_config = {
        "json_schema_extra": {"example": {"jobs": [], "message": "Created 3 training jobs"}}
    }
