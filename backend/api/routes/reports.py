...
class ReportResponse(BaseModel):
    ...
    def from_report(cls, report: Report) -> "ReportResponse":
        ...
        return cls(
            id=str(report.id),
            model_version_id=str(report.model_version_id) if report.model_version_id else "",
            report_type=report.report_type,
            file_size=report.report_metadata.get("file_size", 0) if report.report_metadata else 0,
...
async def generate_report(
    ...
    try:
        ...
        logger.info(
            f"User {current_user.id} generated report {report.id} " f"for model {model_version_id}"
        )
...
async def list_reports(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to list reports"
        )
...
async def get_report(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to retrieve report"
        )
    ...
    try:
        ...
        try:
            pdf_data = storage_client.client.get_object(
                Bucket=storage_client.reports_bucket, Key=report.file_path
            )["Body"].read()
        except Exception as e:
