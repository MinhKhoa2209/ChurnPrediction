...
Usage:
    python scripts/create_r2_buckets.py [--environment ENV]
...
Note: This script requires S3_ENDPOINT_URL, S3_ACCESS_KEY_ID, and S3_SECRET_ACCESS_KEY
...
def get_bucket_names(environment: str = None) -> dict:
    """Get bucket names with optional environment prefix"""
    if environment:
...
def bucket_exists(client, bucket_name: str) -> bool:
    ...
    try:
        ...
        if error_code == '404':
            ...
            # Other errors (403, etc.) - assume bucket exists but we can't access it
...
def create_bucket(client, bucket_name: str) -> bool:
    ...
    try:
        # For R2, we don't specify LocationConstraint
        # R2 uses 'auto' region by default
